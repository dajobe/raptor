...
- Fuzz targets are statically linked against libraptor2 (via libtool
  `-static`) to avoid macOS `dyld: Library not loaded` errors that
  occur when libtool sets the dylib install name to an absolute
  install path.
