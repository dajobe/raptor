 ...
 *
 * raptor_mkr_writer.c - Raptor mKR Writer
 *
 * adapted from raptor_turtle_writer.c Oct/11/2014 Richard H. McCullough
 * use TURTLE flags
...
} raptor_mkr_writer_flags;


#define TURTLE_WRITER_AUTO_INDENT(mkr_writer) ((mkr_writer->flags & TURTLE_WRITER_AUTO_INDENT) != 0)

struct raptor_mkr_writer_s {
  raptor_world* world;
  ...
  /* mKR Writer flags - bits defined in enum raptor_mkr_writer_flags */
  int flags;
...
void
raptor_mkr_writer_increase_indent(raptor_mkr_writer *mkr_writer)
{
  mkr_writer->depth += mkr_writer->indent;
}


void
raptor_mkr_writer_decrease_indent(raptor_mkr_writer *mkr_writer)
{
  mkr_writer->depth -= mkr_writer->indent;
}
...
void
raptor_mkr_writer_newline(raptor_mkr_writer *mkr_writer)
{
  int num_spaces;
  
  raptor_iostream_write_byte('\n', mkr_writer->iostr);
 
  if(!TURTLE_WRITER_AUTO_INDENT(mkr_writer))
    return;

  num_spaces = mkr_writer->depth * mkr_writer->indent;
  ...
  while(num_spaces > 0) {
    ...
    raptor_iostream_counted_string_write(spaces_buffer, count, mkr_writer->iostr);
 ...
 * 
 * Constructor - Create a new mKR Writer writing mKR to a raptor_iostream
 * 
 * Return value: a new #raptor_mkr_writer object or NULL on failure
 **/
raptor_mkr_writer*
raptor_new_mkr_writer(raptor_world* world,
                         ...
                         raptor_iostream* iostr)
{
  ...
  mkr_writer = RAPTOR_CALLOC(raptor_mkr_writer*, 1,
                                sizeof(*mkr_writer));

  if(!mkr_writer)
    return NULL;

  mkr_writer->world = world;

  mkr_writer->nstack_depth = 0;

  mkr_writer->nstack = nstack;
  if(!mkr_writer->nstack) {
    mkr_writer->nstack = raptor_new_namespaces(world, 1);
    mkr_writer->my_nstack = 1;
  }

  mkr_writer->iostr = iostr;

  mkr_writer->flags = 0;
  mkr_writer->indent = 2;

  mkr_writer->base_uri = NULL;
  /* Ensure any initial base URI is not written relative */
  if(base_uri && write_base_uri)
    raptor_mkr_writer_base(mkr_writer, base_uri);
  mkr_writer->base_uri = base_uri;
...
/**
 * raptor_free_mkr_writer:
 * @mkr_writer: mKR writer object
 *
 * Destructor - Free mKR Writer
 * 
 **/
void
raptor_free_mkr_writer(raptor_mkr_writer* mkr_writer)
{
  if(!mkr_writer)
    return;

  if(mkr_writer->nstack && mkr_writer->my_nstack)
    raptor_free_namespaces(mkr_writer->nstack);

  RAPTOR_FREE(raptor_mkr_writer, mkr_writer);
}


static int
raptor_mkr_writer_contains_newline(const unsigned char *s)
{
  size_t i = 0;
...
/**
 * raptor_mkr_writer_raw:
 * @mkr_writer: mKR writer object
 * @s: raw string to write
 *
 * Write a raw string to the mKR writer verbatim.
 *
 **/
void
raptor_mkr_writer_raw(raptor_mkr_writer* mkr_writer,
                         const unsigned char *s)
{
  raptor_iostream_string_write(s, mkr_writer->iostr);
}


/**
 * raptor_mkr_writer_raw_counted:
 * @mkr_writer: mKR writer object
 * @s: raw string to write
 * @len: length of string
 *
 * Write a counted string to the mKR writer verbatim.
 *
 **/
void
raptor_mkr_writer_raw_counted(raptor_mkr_writer* mkr_writer,
                                 const unsigned char *s, unsigned int len)
{
  raptor_iostream_counted_string_write(s, len, mkr_writer->iostr);
}


/**
 * raptor_mkr_writer_namespace_prefix:
 * @mkr_writer: mKR writer object
 ...
 */
void
raptor_mkr_writer_namespace_prefix(raptor_mkr_writer* mkr_writer,
                                      raptor_namespace* ns)
{
  raptor_iostream_string_write("@prefix ", mkr_writer->iostr);
  if(ns->prefix)
 ...
 */
void
raptor_mkr_writer_base(raptor_mkr_writer* mkr_writer,
                          raptor_uri* base_uri)
{
  if(base_uri) {
...
/**
 * raptor_mkr_writer_reference:
 * @mkr_writer: mKR writer object
 * @uri: URI to write
 *
 * Write a mKR-encoded URI to the mKR writer.
 *
 * Return value: non-0 on failure
 **/
int
raptor_mkr_writer_reference(raptor_mkr_writer* mkr_writer, 
                               raptor_uri* uri)
{
  return raptor_uri_escaped_write(uri, mkr_writer->base_uri, 
...
/**
 * raptor_mkr_writer_qname:
 * @mkr_writer: mKR writer object
 * @qname: qname to write
 *
 * Write a QName to the mKR writer.
 *
 **/
void
raptor_mkr_writer_qname(raptor_mkr_writer* mkr_writer,
                           raptor_qname* qname)
{
  raptor_iostream* iostr = mkr_writer->iostr;
...
/**
 * raptor_mkr_writer_quoted_counted_string:
 * @mkr_writer: mKR writer object
 * @s: string to write
 * @len: string length
 *
 * Write a mKR escaped-string inside double quotes to the writer.
 *
 * Return value: non-0 on failure
 **/
int
raptor_mkr_writer_quoted_counted_string(raptor_mkr_writer* mkr_writer,
                                           const unsigned char *s, size_t len)
{
  ...
  /* mKR """longstring""" (2) or "string" (1) */
  if(raptor_mkr_writer_contains_newline(s)) {
    /* long string */
  ...
  raptor_iostream_counted_string_write(q, q_len, mkr_writer->iostr);
  rc = raptor_string_escaped_write(s, strlen((const char*)s), '"', 
                                   flags, mkr_writer->iostr);
  raptor_iostream_counted_string_write(q, q_len, mkr_writer->iostr);
 ...
 *
 * INTERNAL - Write a literal (possibly with lang and datatype) to the mKR writer.
 *
 * Return value: non-0 on failure
 **/
int
raptor_mkr_writer_literal(raptor_mkr_writer* mkr_writer,
                             raptor_namespace_stack *nstack,
                             ...
                             raptor_uri* datatype)
{
  ...
  if(datatype) {
    /* integer */
    if(raptor_uri_equals(datatype, mkr_writer->world->xsd_integer_uri)) {
      ...
      if(endptr != (char*)s && !*endptr) {
        ...
        raptor_log_error(mkr_writer->world, RAPTOR_LOG_LEVEL_ERROR, NULL,
                         "Illegal value for xsd:integer literal.");
      }

    /* double, decimal */
    } else if(raptor_uri_equals(datatype, mkr_writer->world->xsd_double_uri) ||
      raptor_uri_equals(datatype, mkr_writer->world->xsd_decimal_uri)) {
      /* FIXME. Work around that gcc < 4.5 cannot disable warn_unused_result */
    ...
    /* boolean */
    } else if(raptor_uri_equals(datatype, mkr_writer->world->xsd_boolean_uri)) {
      if(!strcmp((const char*)s, "0") || !strcmp((const char*)s, "false")) {
        raptor_iostream_string_write("false", mkr_writer->iostr);
        written = 1;
      } else if(!strcmp((const char*)s, "1") || !strcmp((const char*)s, "true")) {
        raptor_iostream_string_write("true", mkr_writer->iostr);
        written = 1;
      } else {
        raptor_log_error(mkr_writer->world, RAPTOR_LOG_LEVEL_ERROR, NULL,
  ...
  if(raptor_mkr_writer_quoted_counted_string(mkr_writer, s,
                                                strlen((const char*)s)))
    return 1;
  ...
  if(datatype) {
    ...
    raptor_iostream_string_write("^^", mkr_writer->iostr);
    qname = raptor_new_qname_from_namespace_uri(nstack, datatype, 10);
    if(qname) {
      raptor_mkr_writer_qname(mkr_writer, qname);
      raptor_free_qname(qname);
    } else
      raptor_mkr_writer_reference(mkr_writer, datatype);
  } else if(lang) {
    /* literal with language tag */
    raptor_iostream_write_byte('@', mkr_writer->iostr);
    raptor_iostream_string_write(lang, mkr_writer->iostr);
  }
...
/**
 * raptor_mkr_writer_comment:
 * @mkr_writer: mKR writer object
 * @s: comment string to write
 *
 * Write a mKR comment to the mKR writer.
 *
 **/
void
raptor_mkr_writer_comment(raptor_mkr_writer* mkr_writer,
                             const unsigned char *string)
{
  ...
  for(; (c=*string); string++, len--) {
    if(c == '\n') {
      ...
      raptor_iostream_write_byte(c, mkr_writer->iostr);
    }
  }
...
/**
 * raptor_mkr_writer_set_option:
 * @mkr_writer: #raptor_mkr_writer mkr_writer object
 * @option: option to set from enumerated #raptor_option values
 * @value: integer option value (0 or larger)
 *
 * Set mkr_writer options with integer values.
 * 
 ...
 **/
int
raptor_mkr_writer_set_option(raptor_mkr_writer *mkr_writer, 
                                 raptor_option option, int value)
{
  ...
  switch(option) {
    case RAPTOR_OPTION_WRITER_AUTO_INDENT:
      if(value)
        mkr_writer->flags |= TURTLE_WRITER_AUTO_INDENT;
      else
        mkr_writer->flags &= ~TURTLE_WRITER_AUTO_INDENT;        
      break;

    case RAPTOR_OPTION_WRITER_INDENT_WIDTH:
      mkr_writer->indent = value;
      break;
...
/**
 * raptor_mkr_writer_set_option_string:
 * @mkr_writer: #raptor_mkr_writer mkr_writer object
 * @option: option to set from enumerated #raptor_option values
 * @value: option value
 *
 * Set mkr_writer options with string values.
 * 
 * The allowed options are available via
 * raptor_world_get_option_description().
 ...
 **/
int
raptor_mkr_writer_set_option_string(raptor_mkr_writer *mkr_writer, 
                                        ...
                                        const unsigned char *value)
{
  ...
  if(raptor_option_value_is_numeric(option))
    return raptor_mkr_writer_set_option(mkr_writer, option, 
...
/**
 * raptor_mkr_writer_get_option:
 * @mkr_writer: #raptor_mkr_writer serializer object
 * @option: option to get value
 *
 * Get various mkr_writer options.
 * 
 ...
 **/
int
raptor_mkr_writer_get_option(raptor_mkr_writer *mkr_writer, 
                                 raptor_option option)
{
  int result = -1;
  ...
  switch(option) {
    case RAPTOR_OPTION_WRITER_AUTO_INDENT:
      result = TURTLE_WRITER_AUTO_INDENT(mkr_writer);
      break;

    case RAPTOR_OPTION_WRITER_INDENT_WIDTH:
      result = mkr_writer->indent;
      break;
...
/**
 * raptor_mkr_writer_get_option_string:
 * @mkr_writer: #raptor_mkr_writer serializer object
 * @option: option to get value
 *
 * Get mkr_writer options with string values.
 * 
 * The allowed options are available via raptor_options_enumerate().
 *
 ...
 **/
const unsigned char *
raptor_mkr_writer_get_option_string(raptor_mkr_writer *mkr_writer, 
                                        raptor_option option)
{
  return NULL;
}
...
/**
 * raptor_mkr_writer_bnodeid:
 * @mkr_writer: mKR writer object
 * @bnodeid: blank node ID to write
 * @len: length of @bnodeid
 *
 * Write a blank node ID with leading _: to the mKR writer.
 *
 **/
void
raptor_mkr_writer_bnodeid(raptor_mkr_writer* mkr_writer,
...
/**
 * raptor_mkr_writer_uri:
 * @mkr_writer: mKR writer object
 * @uri: uri
 *
 * Write a #raptor_uri to a mkr writer in qname or URI form
 *
 * Return value: non-0 on failure
 */
int
raptor_mkr_writer_uri(raptor_mkr_writer* mkr_writer,
                         raptor_uri* uri)
{
  raptor_qname* qname;
  int rc = 0;
  ...
  qname = raptor_new_qname_from_namespace_uri(mkr_writer->nstack, uri, 10);

  /* XML Names allow leading '_' and '.' anywhere but mKR does not */
  ...
  if(qname) {
    ...
    rc = raptor_mkr_writer_reference(mkr_writer, uri);
  }
...
/**
 * raptor_mkr_writer_term:
 * @mkr_writer: mKR writer object
 * @term: term
 *
 * Write a #raptor_term to a mkr write
 *
 * Return value: non-0 on failure
 */
int
raptor_mkr_writer_term(raptor_mkr_writer* mkr_writer,
                          raptor_term* term)
{
  int rc = 0;
  ...
  if(term->type == RAPTOR_TERM_TYPE_URI) {
    rc = raptor_mkr_writer_uri(mkr_writer, term->value.uri);
  } else if(term->type == RAPTOR_TERM_TYPE_LITERAL) {
    rc = raptor_mkr_writer_literal(mkr_writer,
                                      mkr_writer->nstack,
...
main(int argc, char *argv[]) 
{
  ...
  mkr_writer = raptor_new_mkr_writer(world, base_uri, 1, nstack, iostr);
  if(!mkr_writer) {
    fprintf(stderr, "%s: Failed to create mkr_writer to iostream\n", program);
    exit(1);
  }
  ...
  raptor_mkr_writer_namespace_prefix(mkr_writer, ex_ns);
  ...
  raptor_mkr_writer_raw(mkr_writer, (const unsigned char*)"ex:foo ");

  raptor_mkr_writer_quoted_counted_string(mkr_writer, longstr,
...
#if defined(RAPTOR_DEBUG) && RAPTOR_DEBUG > 1
  fprintf(stderr, "%s: Made mKR string of %d bytes\n", program, (int)string_len);
  fputs("[[", stderr);
  (void)fwrite(string, 1, string_len, stderr);
  fputs("]]\n", stderr);
#endif
